...
// Version:      ===> 8.2.1 <=== CHANGE SCRIPTVERSION BELOW!
...
// 13-May-19 rbd    8.2.1 (8.2.1) - GEM:1564 Revert the above change due to 
//                  reduced quality of preview. 
// 05-Jun-19 rbd    8.2.1 (8.2.1) - GEM:1696 Fix edge condition in WaitInLimits
//                  by increasing "slop" time at end of wait from 10 sec. to 5 min.
// 06-Jun-12 rbd    8.2.1 (8.2.1) - GEM:1696 Redux. Bad idea, instead add one
//                  degree to the margin. Thanks to Morgenthaler for the wake-up.
//------------------------------------------------------------------------------

//
// Constants
//
var SCRIPTVERSION = "8.2.1";
                        ...
                        if(!Prefs.DisableFinalSolve)
                        {
                            ...
                            else
                            {
                                if(!finalImgSolveFail)                      // Unless we already failed a final image solve
                                {
                                    ...
                                    if(SUP.SolvePlate(imgFile + ".fts", Tc.RA, Tc.Dec, Tc.PA, (SUP.PlateScaleH * Camera.BinX),
...
function WaitInLimits(Tc, Pl)
{
    ...
    for(t = tNow; t <= tEnd; t = t.addSeconds(60))                          // 1 minute steps
    {
        ...
        if(tvec.Elevation < Util.Prefs.MinimumElevation + 1)                // Add a 1 degree slop factor (typ.)
            bInLimits = false;
        else if(tvec.Elevation < Util.Prefs.GetHorizon(tvec.Azimuth) + 1)
            bInLimits = false;
        else
        {
