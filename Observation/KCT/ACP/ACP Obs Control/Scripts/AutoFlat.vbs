...
'
' Version:      ==> 8.2.5 <==  CHANGE SCRIPTVERSION BELOW!
...
'                   for panel flats. 
' 25-Jan-19 rbd     8.2.5 (8.2a) GEM:1670 Fix substitution on LogTemplate
'----------------------------------------------------------------------------
'
Option Explicit

Const SCRIPTVERSION = "8.2.5"                                   ' Script version for logging
...
Sub Main()
    ...
    Dim defLogDir, defPlanDir, defScriptDir, orig
    ...
    If LogFolder = "" Then                                      ' If didn't specify an autoflat log folder
        ...
        End If
    Else
        defLogDir = Prefs.LocalUser.DefaultLogDir & "\AutoFlat"
        LogFolder = Substitute(LogFolder, defLogDir, "", 1, 0, 1, "")
        SUP.CreateFolder LogFolder, False                       ' No ASP
    End If
