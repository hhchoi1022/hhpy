...
def rtsmaker(observatory        = 'KCT',
    ...
    moonseperation  = 40.
    moonphase       = True
    # If moonphase is True, moonseperation is flexible (0.5*moonseperation when phase =0 , 1*moonseperation when phase = 1), added by Hyeonho Choi 
    sunlimit        = '-18'
    '''
    #------------------------------------------------------------#
    #    OBSERVATORY INFO.
    ...
    tdata    = ascii.read(catpath, format = 'fixed_width')
    ...
    filters  = tdata['filters']
    exptime  = tdata['exptime']
    counts   = tdata['counts']
    binning  = tdata['binning']
    #rank     = tdata['rank']            
    dist     = tdata['dist']
    amaj     = tdata['maxaxis']
    bmin     = tdata['minaxis']
    ...
    for i in range(int(endmjd-startmjd)):
        ...
        f.write('#------------------------------------------------------- \n')
        f.write("name ra dec rise(LT) transit(LT) set(LT) moon_dist(deg) distance(Mpc) amaj['] bmin['] priority exptime filter counts binning\n")
        numcount    = 0
        for n in range(len(rad)):
            ...
            if rtscal[0]== None:
                ...
                if numcount < numlimit:
                    ...
                    vis = objname[n] + '\t' + nra + '\t' + ndec + '    ' + rtp + '    ' + ttp + '    ' + stp + '    ' +str(int(msep[n])) + '     ' + str(int(dist[n]))+ '    '+str(amaj[n])+'   '+str(bmin[n])+'   '+str(prior[n])+'   '+str(nexptime)+'   '+str(nfilter)+'   '+str(ncounts)+'   '+str(nbinning)+'\n'
                    f.write(vis)
                ...
                '''
        f.close()
    return save_path+headname+'-'+stryear+strmonth+strday+"-rts_vis-"+observatory+".txt"
#%%