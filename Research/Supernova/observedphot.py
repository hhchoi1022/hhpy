...
class ObservedPhot:
    ...
    data_tbl : str
        ...
        - 'UL5_4' : bool or str
            Flag to indicate uncertainty of upper limit
        - 'e_UL5_4' : float or int
            Error in upper limit
    ...
    def __init__(self,
                 ...
                 target = 'SN2021aefx',
                 observer : str = None,
                 MW_extinction_corrected : bool = None,
                 Host_extinction_corrected : bool = None
                 ):
        
        self._observer = observer
        self._target = target
        self._mwextinction = MW_extinction_corrected
        self._hostextinction = Host_extinction_corrected
        self.data = data_tbl
        
    def __repr__(self) -> str:
        return (f'(target={self._target},'
                f' mwextinction={self._mwextinction}),'
                f' hostextinction={self._hostextinction}),'
    ...
    def get_all_filter(self):
        ...
        filt_array = np.array(list(load_filt_keys()[1].keys()))
        for filt_ in list(load_filt_keys()[1].keys()):
    ...
    def get_data_detected(self,
                          key = 'detected'):
        return self.data[self.data['status'] == key] 
    ...
    def get_marker_observatory(self):
        ...
        markers = load_marker_keys(len(observatories))
        marker_keys = dict()
    ...
    def get_color_filter(self):
        ...
        colors, _, _, _, _ = load_filt_keys(filters)
        return colors
    
    def get_label_filter(self):
        from HHsupport_analysis import load_filt_keys
        filters = self.get_all_filter()
        _, _, _, _, labels = load_filt_keys(filters)
        return labels
    
    def get_offset_filter(self):
        from HHsupport_analysis import load_filt_keys
        filters = self.get_all_filter()
        _, offsets, _, _, _ = load_filt_keys(filters)
        return offsets
    ...
    def exclude_observatory(self,
        ...
        removed_tbl = remove_rows_table(self.data, column_key = 'observatory', remove_keys = observatory)
    ...
    def exclude_filter(self,
        ...
        removed_tbl = remove_rows_table(self.data, column_key = 'filter', remove_keys = filter_)
        self.data = removed_tbl
    ...
    def show_lightcurve(self,
                        ...
                        UL_linelength_ver : float = 1,
                        UL_linelength_hor : float = 0.5,
                        UL_headlength : float = 0.5,
                        UL_headwidth : float = 0.7,
                        UL_alpha : float = 0.5):
        ...
        markers = dict(KCT = '^', LSGT = 's', LasCumbres1m = '.',RASA36 = '*')
        ...
        if color_BV | color_gr | color_UB:
                ...
                for observatory in self.get_observatory():
                    ...
                    if len(show_data_detected) > 0:
                        if line:
                            ...
                            ax1.scatter(show_data_detected['obsdate'], show_data_detected['mag']+offsets[filter_], marker = markers[observatory], facecolors = 'none',  edgecolors = colors[filter_], s = scatter_size, linewidth = scatter_linewidth, label = labels[filter_])
                            ax1.scatter(show_data_detected['obsdate'], show_data_detected['mag']+offsets[filter_], marker = markers[observatory], facecolors = colors[filter_],  edgecolors = colors[filter_], s = scatter_size, linewidth = scatter_linewidth, alpha = 0.2)
                            ax1.errorbar(show_data_detected['obsdate'], show_data_detected['mag']+offsets[filter_], show_data_detected['e_mag'] , fmt = 'none', elinewidth = errorbar_linewidth, capsize = errorbar_capsize, c = colors[filter_], capthick = errorbar_linewidth)
                    if UL:
                        if len(show_data_ul) > 0:
                            #ax1.scatter(show_data_ul['obsdate'], show_data_ul['mag']+offsets[filter_], marker = markers[observatory], facecolors = 'none', edgecolors = colors[filter_], s = scatter_size, linewidth = scatter_linewidth, label = labels[filter_], alpha = 0.2)
                            #ax1.scatter(show_data_ul['obsdate'], show_data_ul['mag']+offsets[filter_], marker = markers[observatory], facecolors = colors[filter_], edgecolors = colors[filter_], s = scatter_size, linewidth = scatter_linewidth, alpha = 0.2)
                            #ax1.errorbar(show_data_ul['obsdate'], show_data_ul['mag']+offsets[filter_], show_data_ul['e_mag'] , fmt = 'none', elinewidth = errorbar_linewidth, capsize = 3, c = colors[filter_], capthick = errorbar_linewidth)
            ...
            if color_UB:
                BV_tbl = match_table(filt_data['U'], filt_data['B'], key = 'obsdate', tolerance = 0.15)
                BV_tbl['e_color'] = (BV_tbl['e_mag_1'] **2 + BV_tbl['e_mag_2'] **2)**(1/2)
                ax2.scatter(BV_tbl['obsdate_1'], BV_tbl['mag_1']-BV_tbl['mag_2'], label = 'U-B', facecolors = 'none',  edgecolors = 'cyan', s = scatter_size, linewidth = scatter_linewidth)
                ax2.errorbar(BV_tbl['obsdate_1'], BV_tbl['mag_1']-BV_tbl['mag_2'],  BV_tbl['e_color'] , fmt = 'none', elinewidth = errorbar_linewidth, capsize = errorbar_capsize, c='cyan', capthick = errorbar_linewidth)
            if color_BV:
                BV_tbl = match_table(filt_data['B'], filt_data['V'], key = 'obsdate', tolerance = 0.15)
                BV_tbl['e_color'] = (BV_tbl['e_mag_1'] **2 + BV_tbl['e_mag_2'] **2)**(1/2)
                ax2.scatter(BV_tbl['obsdate_1'], BV_tbl['mag_1']-BV_tbl['mag_2'], label = 'B-V', facecolors = 'none',  edgecolors = 'b', s = scatter_size, linewidth = scatter_linewidth)
                ax2.errorbar(BV_tbl['obsdate_1'], BV_tbl['mag_1']-BV_tbl['mag_2'],  BV_tbl['e_color'] , fmt = 'none', elinewidth = errorbar_linewidth, capsize = errorbar_capsize, c='b', capthick = errorbar_linewidth)
            if color_gr:
                ...
                ax2.scatter(gr_tbl['obsdate_1'], gr_tbl['mag_1']-gr_tbl['mag_2'], label = 'g-r', facecolors = 'none',  edgecolors = 'g', s = scatter_size, linewidth = scatter_linewidth)
                ax2.errorbar(gr_tbl['obsdate_1'], gr_tbl['mag_1']-gr_tbl['mag_2'],  gr_tbl['e_color'] , fmt = 'none', elinewidth = errorbar_linewidth, capsize = errorbar_capsize, c = 'g', capthick = errorbar_linewidth)
                ...
                for observatory in ['KCT','LSGT','RASA36']:#self.get_observatory():
                    ...
                    if len(show_data_detected) > 0:
                        if line:
                            ...
                            #plt.scatter(show_data_detected['obsdate'], show_data_detected['mag']+offsets[filter_], marker = markers[observatory], facecolors = 'none',  edgecolors = colors[filter_], s = scatter_size, linewidth = scatter_linewidth, label = labels[filter_])
                            #plt.scatter(show_data_detected['obsdate'], show_data_detected['mag']+offsets[filter_], marker = markers[observatory], facecolors = colors[filter_],  edgecolors = colors[filter_], s = scatter_size, linewidth = scatter_linewidth, alpha = 0.2)
                            ...
                            #plt.errorbar(show_data_detected['obsdate'], show_data_detected['mag']+offsets[filter_], show_data_detected['e_mag'] , fmt = 'none', elinewidth = errorbar_linewidth, capsize = errorbar_capsize, c = colors[filter_], capthick = errorbar_linewidth)
                    ...
                    if UL:
                        if len(show_data_ul) > 0:
                            #plt.scatter(show_data_ul['obsdate'], show_data_ul['mag']+offsets[filter_], marker = markers[observatory], facecolors = 'none', edgecolors = colors[filter_], s = scatter_size, linewidth = scatter_linewidth, label = labels[filter_])
                            #plt.scatter(show_data_ul['obsdate'], show_data_ul['mag']+offsets[filter_], marker = markers[observatory], facecolors = colors[filter_], edgecolors = colors[filter_], s = scatter_size, linewidth = scatter_linewidth, alpha = 0.2)
                            #plt.errorbar(show_data_ul['obsdate'], show_data_ul['mag']+offsets[filter_], show_data_ul['e_mag'] , fmt = 'none', elinewidth = errorbar_linewidth, capsize = 3, c = colors[filter_], capthick = errorbar_linewidth)
                            for x,y in zip(show_data_ul['obsdate'], show_data_ul['mag']+offsets[filter_]):
...
if __name__ =='__main__':    
    ...
    observed_data = ObservedPhot(tbl_all, MW_extinction_corrected= True, Host_extinction_corrected= True)
